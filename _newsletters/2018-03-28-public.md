...
## Listing Recommendations and Real-time Personalization in Search at Airbnb (<- TODO: shorter title)

Airbnb has given us a rich [account](https://medium.com/airbnb-engineering/listing-embeddings-for-similar-listing-recommendations-and-real-time-personalization-in-search-601172f7603e) of how they have made use of embeddings for their recommendation systems, an approach we discussed in our most recent [report] and cover in a recent blog [post](http://blog.fastforwardlabs.com/2018/01/22/exploring-recommendation-systems.html). Embeddings are major part of semantic recommendation systems, which offer a solution to the cold start problem endemic to collaborative filtering recommenders. Collaborative filtering recommenders make recommendations (between books and customers, for example) by looking at other shoppers with similar purchasing histories, and then recommending books from other customer's histories to a potential buyer. The intuition behind collaborative filtering is that if you like some of the same things other people like, you'll also like the other things those people like. But what if no one has bought a book yet, because it's a new release? How do you know who to recommend it to? Wouldn't it be great if you could take it's genre, style, and plot into account and base your recommendation off of those qualities? 

Semantic recommendations are able to do just this by taking those qualities and embedding them in multi-dimensional representations of a book, song, or apartment rental. Airbnb uses embeddings to populate their carousel of recommendations to users with alternate listings that match not only the size, price, and location of a listing a user clicked on, but also match stylistic features like decor, architectural type, and amenities of what users are looking for. Read their great [post](https://medium.com/airbnb-engineering/listing-embeddings-for-similar-listing-recommendations-and-real-time-personalization-in-search-601172f7603e) for more, particularly how they evaluated the effectiveness of using this approach for improving their personalized recommendation system, and how they deployed it to work in real time. You can also look [here](https://blog.acolyer.org/2016/04/21/the-amazing-power-of-word-vectors/) and [here](http://blog.fastforwardlabs.com/2016/12/08/dimensionality-reduction-and-intuition.html) for more thoughts on the many great uses of embeddings.
