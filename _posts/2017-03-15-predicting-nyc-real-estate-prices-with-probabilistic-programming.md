...
In price mode, the strength of the orange shading is determined by the median price for each neighborhood. As a heat map showing the relative neighborhood real estate prices, this is the prototype's most conventional view. You can see that the most expensive neighborhoods are clustered in Manhattan, with nearby Brooklyn and Queens neighborhoods trailing closely in price. The map view is complemented by the sidebar, which provides a list of the neighborhoods ordered by price. You can use the sidebar to quickly spot things like the most expensive neighborhood (the Upper East Side).
...
The possibilities of utilizing the entire probability distribution are further explored in the prototype's other, less conventional, mode: probability mode. Price mode gives you a sense of real estate price dynamics across the city. Probability mode is more focused. It has you choose an amount of money and then uses each neighborhood's probability distribution to show you your chances of being able to afford a property within that neighborhood.
...
The probability calculation is visualized very clearly in the neighborhood detail graph. The black line is the selected price, all samples less than that price (to the left of it) you could afford. The probability that you could afford a property within that neighborhod, then, is the samples that are less than the selected price divided by the total number of samples. You can use the graph to see that how much paying an additional $100,000 improves your chances depends on where the selected price is in the distribution. In Prospect Heights in 2016, going from $1.1M to $1.2M improves your chance of affording a property by 2% (from 2% to 4%), while going from $1.7M to $1.8M increases your chance by 7% (from 35% to 43%).
